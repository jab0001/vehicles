...
const { InsurancesDashboard, fetchInsurancesDashboardLoading } = storeToRefs(useDashboardStore());
...
<template>
  <ul
    ...
    v-loading="fetchInsurancesDashboardLoading"
  >
    <li
      v-for="insurance in InsurancesDashboard"
      :key="insurance.vehicle_id"
    ...
    >
      <div class="text-[14px] font-medium flex gap-2 items-center">
        ...
        <p class="font-medium">
              {{ insurance.car_name }}
              <span class="font-medium" v-if="insurance.plate_number"
                >,{{ insurance.plate_number }}</span
              >
            </p>
      </div>
      <ul class="flex justify-between pr-2">
        <li class="flex flex-col gap-1 text-xs" :class="daysUntilWithColor(insurance.osago?.expires_at).color">
          <p>ОСАГО: {{ formatDate(insurance.osago?.expires_at) }}</p>
          <p>{{ daysUntilWithColor(insurance.osago?.expires_at).text }}</p>
        </li>
        <li class="flex flex-col gap-1 text-xs" :class="daysUntilWithColor(insurance.kasko?.expires_at).color">
          <p>КАСКО: {{ formatDate(insurance.kasko?.expires_at) }}</p>
          <p>{{ daysUntilWithColor(insurance.kasko?.expires_at).text }}</p>
        </li>
        <li class="flex flex-col gap-1 text-xs" :class="daysUntilWithColor(insurance.osgop?.expires_at).color">
          <p>ОСГОП: {{ formatDate(insurance.osgop?.expires_at) }}</p>
          <p>{{ daysUntilWithColor(insurance.osgop?.expires_at).text }}</p>
        </li>
      </ul>
    </li>
  </ul>
